...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        const logsResponse = await fetch('http://10.0.15.124:8000/api/logs');
        if (!logsResponse.ok) {
            throw new Error(`Error loading the list of boards: ${logsResponse.statusText}`);
        }
        const logsResponseData = await logsResponse.json();
        allLogsData = Object.entries(logsResponseData).map(([board, count]) => ({board, count}));
        filteredRows = [...allLogsData];

        if (boardName) {
            const timelogResponse = await fetch(`http://10.0.15.124:8000/api/logs/${encodeURIComponent(boardName)}`);
            if (!timelogResponse.ok) {
                throw new Error(`Error loading data: ${timelogResponse.statusText}`);
            }

            const selectedBoard = allLogsData.find(item => item.board === boardName);
            if (selectedBoard) {
                activeBoard = {
                    board: boardName,
                    timestampRow: null,
...
function renderTable() {
    ...
    rowsToShow.forEach(item => {
        ...
        row.addEventListener('click', async (event) => {
            ...
            try {
                const timelogResponse = await fetch(`http://10.0.15.124:8000/api/logs/${encodeURIComponent(item.board)}`);
                if (!timelogResponse.ok) {
                    throw new Error(`Ошибка загрузки данных по плате: ${timelogResponse.statusText}`);
                }
                const timestamps = await timelogResponse.json();
                ...
                const renderFilteredData = (startDate = null, endDate = null) => {
                    ...
                    timestamps.forEach(timestampStr => {
                        ...
                        const timeWithoutMicro = timePart.split('.')[0];
                ...
                alert(`Не удалось загрузить данные по плате: ${error.message}`);
            }
        });
...
function searchLogs() {
    ...
    if (input) {
        filteredRows = allLogsData.filter(item => {
