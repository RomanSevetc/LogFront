...
/*document.addEventListener('DOMContentLoaded', async function () {
    ...
    try {
        const response = await fetch('../data/PCBA.json');
        if (!response.ok) {
        ...
        displayLogInfoPCBA(logInfo);
        displayTestSteps(testSteps);
        displayLogisticDataPCBA(logisticData);
    } catch (error) {
...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    try {
        const response = await fetch(`http://10.0.15.206:8080/api/v1/pcba?pcbanumber=${encodeURIComponent(tcuNumber)}`);
        if (!response.ok) {
            throw new Error(`Error loading the PCBA log data. ${response.statusText}`);
        }
...
function displayLogInfoPCBA(info) {
...
function displayLogisticDataPCBA(data) {
